...
export const createNewGroup = functions.https.onRequest((request, response) => {
        const params = JSON.parse(request.body);
        const token = params.token;
        const name = params.name;
        const image = params.image;
        const members = params.members as Array<any>;
        console.log(token);
        console.log(name);
        console.log(members);
            ...
            db.collection("groups").add({
                ...
                members.forEach(member => {
                    ...
                    let mailOptions = {
                        ...
                        html:
                            '<p>Please follow this <a href="localhost:3000/' + ref + '">link</a> to join ' + name + ' on GroupSplit!</p>'
                    };
